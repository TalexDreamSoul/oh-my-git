...
const levelGroups = levels.reduce<Array<{ chapter: string; items: Array<{ level: Level; index: number }> }>>((groups, level, index) => {
  ...
  if (last?.chapter === level.chapter) {
    last.items.push({ level, index });
  } else {
    groups.push({ chapter: level.chapter, items: [{ level, index }] });
  }
  return groups;
}, []);

export default function App() {
  const [accountInput, setAccountInput] = useState(() => localStorage.getItem('omg-web-account') ?? '');
  const [account, setAccount] = useState(() => localStorage.getItem('omg-web-account') ?? '');
  const [settingsOpen, setSettingsOpen] = useState(false);
  const [settingsName, setSettingsName] = useState(() => localStorage.getItem('omg-web-account') ?? '');
  const [theme, setTheme] = useState<'dark' | 'light'>(() => (localStorage.getItem('omg-web-theme') === 'light' ? 'light' : 'dark'));
  const [soundEnabled, setSoundEnabled] = useState(() => localStorage.getItem('omg-web-sound') !== 'off');
  ...
  const [levelIndex, setLevelIndex] = useState(() => Number(localStorage.getItem(account ? `omg-web-progress:${account}` : 'omg-web-progress:guest') ?? 0));
  ...
  const [solvedLevels, setSolvedLevels] = useState<string[]>(() => JSON.parse(localStorage.getItem(`${storageKey}:solved`) ?? '[]') as string[]);
  ...
  const refresh = useCallback(
    ...
    [git, level, levelIndex, storageKey]
  );
  ...
  async function openPreview(file: string) {
    try {
      ...
      setPreviewFile(file);
      setPreviewContent(error instanceof Error ? error.message : String(error));
    }
  }
  ...
  async function loadLevel(index = levelIndex) {
    ...
    for (const action of nextLevel.setup) {
      await runAction(git, action);
    }
    ...
    <main className="app-shell" data-theme={theme} style={{ '--terminal-height': `${terminalHeight}px` } as React.CSSProperties}>
      <aside className="level-sidebar">
          ...
          {levelGroups.map((group) => (
            <section className="level-group" key={group.chapter}>
              ...
              {group.items.map(({ level: item, index }) => (
                <button
                  className={`level-item ${index === levelIndex ? 'active' : ''}`}
                  key={item.id}
                  onClick={async () => {
                    playTone('click');
                    setLevelIndex(index);
                ...
                >
                  ...
                  <span>
                    ...
                    {solvedLevels.includes(item.id) && <em>已通过</em>}
                  </span>
                </button>
              ))}
            </section>
        ...
        <div className="account-bar">
          <div className="avatar">{avatarText(account)}</div>
      ...
      <section className="workbench">
        <header className="workbench-header">
          <div>
            <p className="eyebrow">操作台</p>
            <div className="title-row">
              <h2>{level.title}</h2>
              <span className={`difficulty difficulty-${level.difficulty}`} aria-label={`难度：${difficultyLabel(level.difficulty)}`}>
                <i />
                <i />
                <i />
                <b>{difficultyLabel(level.difficulty)}</b>
              </span>
            </div>
          </div>
          <div className="header-stats">
            ...
            <span>纯 CLI {pureCli ? '是' : '否'}</span>
          </div>
          <div className="header-actions">
            {won && <span className="win-badge">完成</span>}
            <button onClick={() => loadLevel()}>重置关卡</button>
            <button
              onClick={() => {
        ...
        <section className="graph-stage">
          ...
          <div className="canvas-wrap">
            <CommitCanvas
              ...
              refs={refs}
              branch={branch}
              onCheckoutCommit={async (oid) => {
                setPureCli(false);
                await git.checkout(oid);
                playTone('click');
                await refresh();
              }}
              onCreateBranch={async (oid) => {
                setPureCli(false);
                const name = window.prompt('新分支名称', `branch-${oid.slice(0, 4)}`)?.trim();
                if (!name) return;
                await git.branch(name, oid);
                playTone('success');
                await refresh();
              }}
            />
        ...
        <div
          ...
          onPointerDown={(event) => {
            ...
            const onMove = (moveEvent: PointerEvent) => {
              setTerminalHeight(Math.max(180, Math.min(560, startHeight - (moveEvent.clientY - startY))));
            };
            const onUp = () => {
        ...
        <section className="terminal-stage">
          <XTermPanel git={git} branch={branch} username={account} injectedCommand={injectedCommand} onAfterCommand={refresh} />
        </section>
      </section>

      <aside className="info-sidebar">
        <section className="info-section description-panel">
          <p className="eyebrow">关卡描述</p>
          <h2>{level.title}</h2>
          <p>{level.description}</p>
          <div className="message-line">{message}</div>
        </section>

        <section className="info-section hint-section">
          <div className="hint-header">
            <h3>提示</h3>
            <button onClick={() => setShowHint((value) => !value)}>{showHint ? '收起提示' : '查看提示'}</button>
          </div>
          {showHint ? (
            <ol className="plain-list ordered">
              {level.tutorial.map((item) => (
                <li key={item}>{item}</li>
              ))}
            </ol>
          ) : (
            <p className="muted">先自己试试看。需要帮助时再展开提示。</p>
          )}
        </section>
        ...
        <section className="info-section hint-section">
          <div className="hint-header">
            <h3>示例命令</h3>
            <button onClick={() => setShowExamples((value) => !value)}>{showExamples ? '收起命令' : '查看命令'}</button>
          </div>
          {showExamples ? (
            <div className="command-list">
              {level.commands.map((item) => (
                <button key={item} onClick={() => {
                  setPureCli(false);
                  setInjectedCommand(`${item} `);
                }}>
                  {item}
                </button>
              ))}
            ...
            <p className="muted">如果卡住了，可以展开参考命令。</p>
          )}
        </section>
        ...
        <section className="info-section">
          ...
          {files.length === 0 ? (
            ...
            <ul className="status-list">
              {files.map((file) => {
                const fileStatus = status.find((item) => item.filepath === file);
                return (
                  <li key={file}>
                    <code className={`status-badge ${statusClass(fileStatus?.label)}`}>{fileStatus?.label ?? '工作区'}</code>
      ...
      {won && levelIndex < levels.length - 1 && (
        <div className="advance-toast">
          <strong>关卡完成</strong>
          <span>连续按两次 Enter 或两次 Space 进入下一关</span>
        </div>
      )}
        ...
        <div className="modal-backdrop" onClick={() => setSettingsOpen(false)}>
          <form
            ...
            onSubmit={(event) => {
              ...
              if (nextName) {
                localStorage.setItem('omg-web-account', nextName);
                setAccountInput(nextName);
                setAccount(nextName);
              }
