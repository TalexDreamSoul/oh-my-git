...
import { LOCAL_ANONYMOUS_NAME, LOCAL_ANONYMOUS_NAME_STORAGE_KEY, LOCAL_ANONYMOUS_STORAGE_KEY, isLocalAnonymousAllowed } from '../lib/localAnonymous';
...
export function LoginForm() {
  ...
  const [localAnonymousAllowed, setLocalAnonymousAllowed] = useState(false);

  useEffect(() => {
    setLocalAnonymousAllowed(isLocalAnonymousAllowed());
  ...
  function startLocalAnonymous() {
    if (!requireAgreement()) return;
    if (!isLocalAnonymousAllowed()) {
      setError('匿名测试只允许在本地开发环境使用。');
      return;
    }
    localStorage.setItem(LOCAL_ANONYMOUS_STORAGE_KEY, 'on');
    localStorage.setItem(LOCAL_ANONYMOUS_NAME_STORAGE_KEY, LOCAL_ANONYMOUS_NAME);
  ...
  return (
    <section className="auth-card oauth-only-card">
      ...
      <div className="oauth-provider-list">
        {localAnonymousAllowed && <button type="button" className="hero-auth-button local-anonymous-button" onClick={startLocalAnonymous}>匿名本地测试</button>}
        {!providersLoaded && <div className="provider-loading">正在加载登录方式...</div>}
        {providersLoaded && providers.length === 0 && !localAnonymousAllowed && <div className="provider-empty">当前没有可用登录方式，请联系管理员配置 OAuth。</div>}
        {providers.map((provider) => <a key={provider.id} className="hero-auth-button" href={provider.loginPath} onClick={(event) => { if (requireAgreement(event)) localStorage.removeItem(LOCAL_ANONYMOUS_STORAGE_KEY); }}>{provider.label}</a>)}
      </div>
