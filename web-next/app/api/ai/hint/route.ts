...
const Body = z.object({
  ...
  completedConditions: z.array(z.string()).max(24).default([])
});
...
export async function POST(request: NextRequest) {
  ...
  if (!parsed.success) return Response.json({ ok: false, error: 'Invalid AI hint request.' }, { status: 400 });

  const level = levelById.get(parsed.data.levelId);
  ...
  if (!providerUrl || !providerToken) {
    const fallback = fallbackHint(level.id, parsed.data.completedConditions);
    ...
    const providerResponse = await fetch(providerUrl, {
      ...
      body: JSON.stringify({
        ...
        player: {
          ...
          completedConditions: parsed.data.completedConditions
        },
