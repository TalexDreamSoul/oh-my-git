...
const gitSubcommands = ['add', 'apply', 'bisect', 'blame', 'branch', 'cat-file', 'checkout', 'cherry-pick', 'commit', 'config', 'diff', 'fetch', 'format-patch', 'ignore', 'init', 'log', 'merge', 'pull', 'push', 'rebase', 'recover', 'reflog', 'remote', 'reset', 'restore', 'rm', 'sparse-checkout', 'stash', 'status', 'submodule', 'switch', 'tag', 'worktree'];
...
export async function completeLine(git: BrowserGit, line: string): Promise<{ line: string; suggestions: string[] }> {
  ...
  if (tokens[0] === 'git') {
    ...
    const completingPath = ['add', 'apply', 'blame', 'rm', 'restore', 'reset'].includes(subcommand);
